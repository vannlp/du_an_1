...
class timkiemModel extends DB
{
    function tim_kiem($timkiem)
    {
        $sql = "SELECT * FROM san_pham sp JOIN danhmuc_sp dmsp ON sp.id_dm_sp=dmsp.Id_dm_sp
                WHERE sp.tieu_de LIKE '%$timkiem%' and sp.trang_thai = 1 OR dmsp.tieu_de LIKE '%$timkiem%' and sp.trang_thai = 1";
    ...
    {
        $sql = "SELECT * FROM san_pham WHERE gia_giam <= '$id' and trang_thai = 1";
        return $this->getData($sql);
    }
}
